...
const EditCompetitionPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchCompetition = async () => {
            try {
                const response = await axios.post(CommonConstant.GetCompetitionById, { id: Number(id) });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const request = await axios.post(CommonConstant.EditCompetition, {
                competition_id: Number(id), title, date, status, type, slot, description
            })
            ...
            <div className="flex flex-col">
                <div className="">
                    ...
                    <form method="POST" onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <button
                            type="submit"
                            className="w-fit bg-green-500 text-white mt-5 py-2 px-4 rounded-md duration-300 font-semibold hover:bg-green-600 hover:duration-300"
                        >
                            Save
                        </button>
                    </form>
